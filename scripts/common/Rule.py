...
class Pattern():
    ...
    def isValid(self, testCenter):
        ...
        if not re.match(self.getCenter(), testCenter, flags=re.UNICODE):
            raise RuleException('Non matching center %s, should be %s!' % (testCenter, self.getCenter()))
    ...
    def getCenter(self):
        """Return center in byte encoded string.
        """
        return self.center.encode('utf-8')

    def getPrevContext(self):
        """The previous context in byte encoded string.
        """
        if self.prevContext == None:
            return None
        return self.prevContext.encode('utf-8')

    def getNextContext(self):
        """The next context in byte encoded string.
        """
        if self.nextContext == None:
            return None
        return self.nextContext.encode('utf-8')
    ...
    def getWord(wordsList, indice):
        ...
        if type(strWord) is str:
            strWord = unicode(strWord,'utf-8')
...
class Rule():
    ...
    def display(self,sep=''):
        ...
        strDisplay = prefix + sep + "  --> Rule 1 %s\n" % self.rule1.display(sep + "  ")

        #Depth recursion on rule2
        if self.rule2 != None:
            strDisplay += sep + "  --> Rule 2 %s" % self.rule2.display(sep + "  ")
    ...
    def matchRules(rulesList, wordsList, indice, debug=False):
        ...
        for r in rulesList:
            if not hasattr(r, 'doesApply'):
                raise RuleException("A rule cannot be a pattern. Wrap the pattern instead!")
    ...
    def displayList(wordsList, indice=-1):
        ...
        for indice in range(len(wordsList)):
            strList += Pattern.getWord(wordsList, indice).encode('utf-8') + ", "
        strList += "]"
