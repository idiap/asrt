...
class Number():
    ...
    def convertNumberIntoLetters(number, bUndInUnits=False, ordinal=False):
        ...
        if varnum > 0:
            ...
            if varlet == "eins":
                varlet = "ein"
            letters = letters + varlet + " million"
            if varlet != "ein":
                letters = letters + "en "
        else:
            letters = letters + " "
        ...
        if ordinal:
            letters = Number.applyOrdinalEndings(letters)
    ...
    def applyOrdinalEndings(letters):
        ...
        if lastLetters.strip() in UNITD2W.values() or lastLetters.strip() == 'eins':
            if lastLetters.strip() == 'ein' or lastLetters.strip() == 'eins':
                ...
                letters = firstletters + ' ' + 'dritte'
            elif lastLetters.strip() == 'acht':
                letters = firstletters + ' ' + 'achte'
            else:
                letters = letters.strip() + 'te'
        else:
            letters = letters.strip() + 'ste'
        ...
        if varnum <= 19:
            if varnum > 0:
                if bUndInUnits:
                    ...
                    if varnum == 1:
                        # ex:eins or hunderteins
                        varlet = varlet + ' ' + UNITD2W[varnum] + 's'
                    else:
                        # ex:dreizehn or hundertdreizehn
                        varlet = varlet + ' ' + UNITD2W[varnum]
        else:
            varnumD = int(varnum / 10)  # quotient
            varnumU = varnum % 10  # reste
            if varnumU != 0:
                # ex: zweiundachtzig or hundertzweiundachtzig
                varlet = varlet + UNITD2W[varnumU] + \
                    " und " + DECADED2W[varnumD]
            else:
